...
class MyAgentProgram implements AgentProgram {
	...
	public void find_nearest_unknown() {															//Find the nearest (from the current x- and y-position) unknown node
		...
		nodes_with_parent.put(starting_node, null);													//Set the parent of the current position to null, used when finding a path to an unknown node
		
		while (!frontier.isEmpty()) {
			next_node = frontier.poll();
			if (state.world[(int)next_node.getX()][(int)next_node.getY()] == state.UNKNOWN			//If the node polled from frontier is unknown, find a path to that node
				&& next_node.getX() >= 0 && next_node.getY() >= 0 									//by adding the parent of the unknown node, the parent of that parent and so forth
				&& next_node.getX() <= 30 && next_node.getY() <= 30) {								//until the parent of the current node is null (the starting node)
				...
				tmp_path_to_target.remove(0);
				for (int y = 0; y < tmp_path_to_target.size(); y++) {								//Add each node to path_to_target 
			...
			for (int x = 0; x < 4; x++) {															//Generate neighbours, disregard if a wall or already exist in nodes_with_parent
				if (x == 0) {																		//The neighbour under the current node (Y-1)
				...
				else if (x == 1) {																	//The neighbour to the right of the current node (X+1)
				...
				else if (x == 2) {																	//The neighbour above the current node (Y-1)
					Point neighbour_node = new Point((int)next_node.getX(), (int)next_node.getY() - 1);
					if (state.world[(int)neighbour_node.getX()][(int)neighbour_node.getY()] != state.WALL
						&& !nodes_with_parent.containsKey(neighbour_node)) {
						nodes_with_parent.put(neighbour_node, next_node);
				...
				else {																				//The neighbour to the left of the current node (X-1)
					Point neighbour_node = new Point((int)next_node.getX() - 1, (int)next_node.getY());
					if (state.world[(int)neighbour_node.getX()][(int)neighbour_node.getY()] != state.WALL
						&& !nodes_with_parent.containsKey(neighbour_node)) {
						nodes_with_parent.put(neighbour_node, next_node);
		...
		System.out.println("Finished vacuuming, return to HOME");									//If the while loop breaks, no valid path to an unknown node was found and the vacuum is finished
	...
	public void go_home() {																			//Function that returns the vacuum to HOME. Executes once there are no paths to an unknown node available
		Vector<Point> explored = new Vector<Point>();												//(same as find_nearest_unknown, but looks for HOME instead of UNKNOWN)
		Queue<Point> frontier = new LinkedList<Point>();
		Map<Point, Point> nodes_with_parent = new HashMap<Point, Point>();
		Point starting_node = new Point(state.agent_x_position, state.agent_y_position);
		Point next_node = new Point(0,0);
	...
	public void fill_action_queue() {																//Simulates the vacuum moving from the current pos to the node found in find_nearest_unknown() or go_home()
		int tmp_x = state.agent_x_position;															//Adds each action done to the action_queue
	...
	public Action execute(Percept percept) {
	    ...
	    if (state.action_queue.isEmpty()) {														//If the unexplored node (found through find_nearest_unknown) is reached
	    	find_nearest_unknown();
	    	fill_action_queue();
	    }
	    
	    if (state.action_queue.isEmpty() && state.finished_vacuuming) {							//If the vacuum finishes exploring when at x=1 and y=1
	    	state.agent_last_action = state.ACTION_NONE;
	    	return NoOpAction.NO_OP;
	    }
	    
	    int next_action = state.action_queue.poll();											//Perform next action in action_queue
